...
static einline unsigned test_c(unsigned i0, unsigned i1, unsigned r, unsigned sub) {
    unsigned flag;
...
einline void inst_psh(unsigned op, unsigned* sp, unsigned data, unsigned* cycles) {
    if (op & 0x80) {
        push16(sp, reg_pc);
        *cycles += 2;
    }
...
einline void inst_pul(unsigned op, unsigned* sp, unsigned* osp, unsigned* cycles) {
    if (op & 0x01) {
        reg_cc = pull8(sp);
        *cycles += 1;
    }
